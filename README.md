...
| `POST` | `/api/v1/simulate/start` | Spin up the synthetic iGrill device. Body (optional): `{"speed": 10, "probes": 4}` — `speed` is the time multiplier (default 10) and `probes` is the number of active probes 1–4 (default 4). The simulator presents itself as a connected device and broadcasts readings; it does not start a session. Returns `deviceAddress`, `speed`, and `probes`. Returns 409 if the simulator is already running. Requires authorisation. |
| `POST` | `/api/v1/simulate/stop` | Tear down the synthetic device and mark it disconnected. Any user-started session remains active. Returns `readings` count. Returns 400 if no simulation is running. Requires authorisation. |
| `GET` | `/api/v1/simulate/status` | Report whether the synthetic device is currently running. Returns `{"running": bool, "address": str}`. Requires authorisation. |
| `POST` | `/api/v1/simulate/probe-timer` | Drive the simulated device's per-probe timer through the same dispatch path a real device uses. Body: `{"probe_index": int, "action": str, "mode"?: str, "duration_secs"?: int}`. `action` is one of `upsert`, `start`, `pause`, `resume`, `reset`. On success broadcasts a `probe_timer_update` envelope and returns the fresh row. Returns 409 if no cook session is active. Requires authorisation. |
