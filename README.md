...
| `POST` | `/api/v1/simulate/start` | Start a simulated cook session. Body (optional): `{"speed": 10, "probes": 4, "probe_timers": {"1": {"mode": "count_up"}, "2": {"mode": "count_down", "duration_secs": 600}}}`. `speed` is the time multiplier (default 10), `probes` is the number of active probes 1-4 (default 4), and `probe_timers` optionally pre-attaches paused per-probe timers so timer flows can be exercised without real hardware. Returns `sessionId`, `speed`, and `probes`. Returns 409 if a simulation is already running. Requires authorisation. |
...
| `POST` | `/api/v1/simulate/probe-timer` | Drive a simulated session's per-probe timer through the same dispatch path the real device uses. Body: `{"probe_index": int, "action": str, "mode"?: str, "duration_secs"?: int}`. `action` is one of `upsert`, `start`, `pause`, `resume`, `reset`. On success broadcasts a `probe_timer_update` envelope and returns the fresh row. Returns 409 if no simulation is running. Requires authorisation. |
